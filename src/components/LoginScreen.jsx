...
export default function LoginScreen({ onLogin }) {
    ...
    const handleLogin = (isGuest = false) => {
        ...
        const mockUser = {
            id: isGuest ? 'guest-' + Math.random().toString(36).substr(2, 9) : 'mock-google-id-' + Math.random().toString(36).substr(2, 9),
            name: isGuest ? 'Guest User' : 'Student ' + Math.floor(Math.random() * 1000),
            email: isGuest ? 'guest@mathmastery.sg' : 'student@gmail.com',
            isNewUser: true,
            isGuest: isGuest,
        };
    ...
    return (
        <div className="flex flex-col items-center justify-center w-full" style={{ minHeight: '80vh', animation: 'fadeIn 0.5s ease' }}>
            <Card style={{ padding: 'var(--space-8)', maxWidth: '450px', width: '100%', textAlign: 'center' }}>
                ...
                {error && (
                    <div style={{ color: 'var(--accent-error)', backgroundColor: 'rgba(239, 68, 68, 0.1)', padding: 'var(--space-3)', borderRadius: 'var(--radius-sm)', marginBottom: 'var(--space-4)', fontSize: '0.9rem', border: '1px solid var(--accent-error)' }}>
                        {error}
                    </div>
                )}
